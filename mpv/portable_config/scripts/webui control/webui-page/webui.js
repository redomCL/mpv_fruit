...
function setFullscreenButton(fullscreen) {
  let fullscreenText = "切换全屏";
  if (fullscreen) {
    fullscreenText = "切换全屏";
  }
  document.getElementById("全屏切换").innerText = fullscreenText;
}
...
function setTrackList(tracklist) {
  ...
  document.getElementById("字幕切换").innerText =
    "字幕切换 " + window.subs.selected + "/" + window.subs.count;
  document.getElementById("音频切换").innerText =
    "音频切换 " + window.audios.selected + "/" + window.audios.count;
}
