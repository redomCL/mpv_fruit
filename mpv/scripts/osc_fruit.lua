...
SOURCE COMMIT_ c84bb1ce67f2dd3adb974c49a66d28c0cf55d39d
...
可用的快捷键示例（在 input.conf 中写入）：
 <KEY>   script-binding osc_fruit/visibility   # 切换 osc_fruit 的可见性

 <KEY>   script-message-to osc_fruit osc-visibility <值>   # <auto|always|never|cycle> 指定 osc_fruit 的可见性
 <KEY>   script-message-to osc_fruit osc-idlescreen <值>   # <yes|no|cycle> 指定空闲状态Logo的可见性
 <KEY>   script-message-to osc_fruit osc_layout <值>       # <bottombar|topbar|bottombox|box|slimbox> 指定 osc_fruit 的布局
]]
...
local osc_styles = {
    ...
    timePosBar = ("{\\blur0\\bord%s\\1c&HFFFFFF\\3c&H000000\\fs30\\b%d\\fn%s}"):format(user_opts.tooltipborder, user_opts.font_bold, user_opts.font_mono),
...
function window_controls_enabled()
    ...
    if val == "auto" then
        return not (state.border and state.title_bar) or state.fullscreen  -- 全屏时启用osc顶部控件
    else
        return val ~= "no"
    end
end
...
function window_controls(topbar)
    ...
    add_area("window-controls-title_osc_fruit",
...
function osc_init()
        ...
        function (element)
            ...
            if element.state.lastseek == nil or
                element.state.lastseek ~= seekto then
                    local flags = "absolute-percent+keyframes" -- 防止 --hr-seek 的影响
                    if not user_opts.seekbarkeyframes then
                        flags = "absolute-percent+exact"       -- 防止 --hr-seek 的影响
                    end
    ...
    -- bottombox的统计信息按钮 联动内置的 stats.lua
...
function render()
    ...
    for k,cords in pairs(osc_param.areas["showhide"]) do
        set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, "showhide_osc_fruit")
    end
    if osc_param.areas["showhide_wc"] then
        for k,cords in pairs(osc_param.areas["showhide_wc"]) do
            set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, "showhide_wc_osc_fruit")
        end
    else
        set_virt_mouse_area(0, 0, 0, 0, "showhide_wc_osc_fruit")
    end
    ...
    for _,cords in ipairs(osc_param.areas["input"]) do
        if state.osc_visible then -- activate only when OSC is actually visible
            set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, "input_osc_fruit")
        end
        if state.osc_visible ~= state.input_enabled then
            if state.osc_visible then
                mp.enable_key_bindings("input_osc_fruit")
            else
                mp.disable_key_bindings("input_osc_fruit")
            end
    ...
    if osc_param.areas["window-controls"] then
        for _,cords in ipairs(osc_param.areas["window-controls"]) do
            if state.osc_visible then -- activate only when OSC is actually visible
                set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, "window-controls_osc_fruit")
            end
            if state.osc_visible ~= state.windowcontrols_buttons then
                if state.osc_visible then
                    mp.enable_key_bindings("window-controls_osc_fruit")
                else
                    mp.disable_key_bindings("window-controls_osc_fruit")
                end
    ...
    if osc_param.areas["window-controls-title_osc_fruit"] then
        for _,cords in ipairs(osc_param.areas["window-controls-title_osc_fruit"]) do
            if state.osc_visible then -- activate only when OSC is actually visible
                set_virt_mouse_area(cords.x1, cords.y1, cords.x2, cords.y2, "window-controls-title_osc_fruit")
            end
            if state.osc_visible ~= state.windowcontrols_title then
                if state.osc_visible then
                    mp.enable_key_bindings("window-controls-title_osc_fruit", "allow-vo-dragging")
                else
                    mp.disable_key_bindings("window-controls-title_osc_fruit", "allow-vo-dragging")
                end
...
function tick()
    ...
    if state.idle then
        ...
        if state.showhide_enabled then
            mp.disable_key_bindings("showhide_osc_fruit")
            mp.disable_key_bindings("showhide_wc_osc_fruit")
...
function do_enable_keybindings()
    if state.enabled then
        if not state.showhide_enabled then
            mp.enable_key_bindings("showhide_osc_fruit", "allow-vo-dragging+allow-hide-cursor")
            mp.enable_key_bindings("showhide_wc_osc_fruit", "allow-vo-dragging+allow-hide-cursor")
        end
...
}, "showhide_osc_fruit", "force")
mp.set_key_bindings({
    {"mouse_move",              function(e) process_event("mouse_move", nil) end},
    {"mouse_leave",             mouse_leave},
}, "showhide_wc_osc_fruit", "force")
do_enable_keybindings()
...
}, "input_osc_fruit", "force")
mp.enable_key_bindings("input_osc_fruit")

mp.set_key_bindings({
    {"mbtn_left",           function(e) process_event("mbtn_left", "up") end,
                            function(e) process_event("mbtn_left", "down")  end},
}, "window-controls_osc_fruit", "force")
mp.enable_key_bindings("window-controls_osc_fruit")
...
set_virt_mouse_area(0, 0, 0, 0, "input_osc_fruit")
set_virt_mouse_area(0, 0, 0, 0, "window-controls_osc_fruit")
set_virt_mouse_area(0, 0, 0, 0, "window-controls-title_osc_fruit")
